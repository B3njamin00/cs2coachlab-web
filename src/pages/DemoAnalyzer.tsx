      ...
      if (data.analysis) {
        setSaveMessage(
          data.wasAlreadySaved
            ? "Kampen og treningsplanen ble oppdatert."
            : "Kampen ble lagret og treningsplanen ble generert."
        );
      }
    } catch (error) {
      ...
      {players.length > 0 && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <button
            type="button"
            onClick={analyzeDemo}
            disabled={!playerName || isAnalyzing}
      ...
      {aiCoach && (
        <div className="rounded-2xl border border-orange-500/30 bg-[#0c1426] p-6">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            ...
            <Stat label="Vunnet" value={aiCoach.openingDuels.won} color="text-green-400" />
            <Stat label="Tapt" value={aiCoach.openingDuels.lost} color="text-red-400" />
            <Stat label="Win rate" value={`${aiCoach.openingDuels.winRate}%`} color="text-orange-400" />
          </div>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            <div className="rounded-xl bg-slate-900 p-5">
              ...
              <p className="mt-5 text-sm uppercase text-slate-400">Neste kamp</p>
            ...
            {aiCoach.utility && (
              <div className="rounded-xl bg-slate-900 p-5">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                  <UtilityStat label="Flashes" value={aiCoach.utility.stats.flashesThrown} />
                  <UtilityStat label="Smokes" value={aiCoach.utility.stats.smokesThrown} />
                  <UtilityStat label="HE" value={aiCoach.utility.stats.heGrenadesThrown} />
                  <UtilityStat label="Molotov" value={aiCoach.utility.stats.molotovsThrown} />
                  <UtilityStat label="Blind-events" value={aiCoach.utility.stats.blindEventsCreated} />
                  <UtilityStat
                    label="Util damage"
                    value={aiCoach.utility.stats.heDamage + aiCoach.utility.stats.molotovDamage}
                  />
                </div>
              </div>
            )}
